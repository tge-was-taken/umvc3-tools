...
DEPENDENCIES = ['--upgrade pip', 'pyyaml', 'ruamel.yaml', 'numpy']
...
def findSupportedMaxInstallDirs():
    ...
    for dir in os.scandir( autodeskDir ):
        if dir.is_dir() and dir.name.startswith( '3ds Max ' ):
            verStr = dir.name.replace( '3ds Max ', "" )
            ver = None
            try:
                ver = int( verStr )
            except:
            ...
            if ver < MIN_SUPPORTED_VER:
                # no python 3 support
                print( f"notice: skipping incompatible 3ds Max installation (ver: {ver}, required: >= {MIN_SUPPORTED_VER})" )
                continue
